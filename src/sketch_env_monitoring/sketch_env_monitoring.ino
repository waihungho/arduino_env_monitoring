...
//        b. Read data from PMS3003 [readPMS3003()]
//        c. Display data on LCD    [displayDataOnLCD()]
//
//    Remark: 
//      If you got "Low Memory Available" error when compiling this program, comment out all Serial.println("xxxx") code.
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#define READING_SENSOR_INTERVAL 10000     // Interval to read ALL sensors.
#define ENABLE_CLOCK_DS1302 1             // 1: Enable Clock, 0; Disable Clock.
#define DISPLAY_CLOCK_DS1302_ON_LED 0     // 1: Show time on LED; 0: Dont show.
#define ENABLE_SD_CARD 1                  // 1: Enable SD Card, 0; Disable.
#define SD_CARD_FILENAME "MONITOR.TXT"    // SD Card Format: FAT16,  Filename format: XXXXXXXX.txt (max:8 chars).
// ------------------------------------------
...
void loop() { // put your main code here, to run repeatedly:
  ...
  readPMS3003(); // read PM2.5 sensor. put it into variables (pmcf10, .., pmat100).
...
void writeToSDCard(){
  if ( ENABLE_SD_CARD && isSuccesToInitSD){
    ...
    if ( sdCardFile ) {
      ...
      char timeStr[50];   
      snprintf(timeStr, sizeof(timeStr), "%04d/%02d/%02d %02d:%02d:%02d", currentTime.yr, currentTime.mon, currentTime.date, currentTime.hr, currentTime.min, currentTime.sec);
      ...
      if (firstLoop){
        sdCardFile.println(String(SD_FILE_DATA_HEADER));  // Write Header
      }
...
void displayDataOnLCD(){
  ...
  if(ENABLE_CLOCK_DS1302 && DISPLAY_CLOCK_DS1302_ON_LED && isSuccesToInitSD){
    ...
    snprintf(buf, sizeof(buf), " %02d/%02d %02d:%02d",clockTime.mon, clockTime.date, clockTime.hr, clockTime.min);
    firstLine += buf;
  }
